...
const { sendMailUser } = require("../controller/mail");
...
router.post("/forgotpassword", middleware, async (req, res) => {
  const body = req?.body;
  const email = body?.email;
  // console.log(email);
  try {
    await sendMailUser(email, "Change Password");
    res.send("done");
  } catch (err) {
    console.log(err);
  }
});
router.post("/changepassword", middleware, async (req, res) => {
  const body = req?.body;
  const email = body?.email;
